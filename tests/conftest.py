...
def collect_tests_files():
    """Collect .tests files from bundled data and optional external source."""
    cases = []
    if os.path.isdir(BUNDLED_DATA):
        for f in sorted(os.listdir(BUNDLED_DATA)):
            if f.endswith(".tests"):
                cases.extend(parse_tests_file(os.path.join(BUNDLED_DATA, f)))
    if HARFRUST_SOURCE:
        ...
        if os.path.isdir(ext_tests):
            for f in sorted(os.listdir(ext_tests)):
                if f.endswith(".tests"):
                    cases.extend(parse_tests_file(os.path.join(ext_tests, f)))
    return cases


def pytest_generate_tests(metafunc):
    if "tests_case" in metafunc.fixturenames:
        cases = collect_tests_files()
        ids = [f"{os.path.basename(c[0])}:{c[1][:20]}" for c in cases]
        metafunc.parametrize("tests_case", cases, ids=ids)
